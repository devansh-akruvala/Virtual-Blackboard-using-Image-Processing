...
while True:
    ...
    page = cv2.rectangle(page, (0, fh-50), (0 + 50, fh), [255, 0, 255], 5)
    page = cv2.rectangle(page, (0, fh-120), (0 + 50, fh-70), [255, 0, 255], 5)
    ...
    if(flag==True):
        ...
        if contour and cv2.contourArea(max(contour,key=cv2.contourArea))>100:
            ...
            if (flcalculated == 0 and cv2.waitKey(1) & 0xFF == ord('m')):
                ...
                flcalculated = 1
            if (flcalculated == 1):
                distance=calc_dist(known_width,focal_length,w)
                print("Distance is: ",distance)
            if (x2 < 60 and y2 > fh - 60 and distance<50):
                page = None
                continue
            elif x2 < 60 and y2 > fh - 120 and y2 <fh-70 and distance<50 :
                if switch=="pen":
                    switch="earser"
                else:
                    switch="pen"
                ...
                if(switch=="pen" and distance<50):
                    ...
                    if distance<50:
                        cv2.circle(page, (x2, y2), 20, (0, 0, 0), -1)
